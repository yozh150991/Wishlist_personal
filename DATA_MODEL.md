...
| `items_list_price_idx (list_id, price, id)` | **фільтр** діапазону ціни. Не дублікат попереднього: сортування йде за виразом `coalesce(price, -1)`, і планувальник не виводить із нього умову на голий `price` — тому фільтр без цього індексу вироджується в обхід індексу сортування з відкиданням рядків (ADR-030) |
