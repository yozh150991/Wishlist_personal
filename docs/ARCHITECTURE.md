...
- **Читання офлайн (етап 6.4).** Успішне завантаження сторінки списків і першої партії позицій лишає знімок в IndexedDB (`lib/cache.ts`). Коли мережі немає, замість «Немає зʼєднання» показується цей знімок із позначкою, коли його збережено (`StaleNotice`). Кожен запис підписаний `userId`: чужий підпис — промах, а вихід з акаунта стирає базу цілком. У кеш не потрапляють ні токени спільних посилань, ні сторінка `/shares`, ні броні, ні вибірки з пошуком чи фільтрами (ADR-028).
