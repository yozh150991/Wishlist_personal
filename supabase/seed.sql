...
-- Ознаки (ADR-030) — окремим оновленням, щоб не подовжувати список колонок
-- вище. Дві позиції з ознаками: гостьова сторінка й експорт перевіряються на
-- реальних даних, а не лише на порожньому масиві.
update public.items
   set variants = '[{"label":"Колір","value":"чорний"},{"label":"Модель","value":"WH-1000XM5"}]'::jsonb
 where id = '5eed0000-0000-4000-8000-00000000c001';

update public.items
   set variants = '[{"label":"Обʼєм","value":"450 мл"}]'::jsonb
 where id = '5eed0000-0000-4000-8000-00000000c002';
